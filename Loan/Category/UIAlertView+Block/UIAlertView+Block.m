...
- (void)showAlertViewWithDismissCompleteBlock:(CompleteBlock)block
{
    if (block) {
        ...
        /**
         ...
         3 关联策略表明了相关的对象是通过赋值，保留引用还是复制的方式进行关联的；关联是原子的还是非原子的。这里的关联策略和声明属性时的很类似。
         */
        objc_setAssociatedObject(self, &didDismissBlock, block, OBJC_ASSOCIATION_COPY);
        ////设置delegate
        self.delegate = self;
    }
    [self show];
...
{
    ///获取关联的对象，通过关键字。
    CompleteBlock block = objc_getAssociatedObject(self, &didDismissBlock);
    if (block) {
        ///block传值
        block(buttonIndex);
    }
